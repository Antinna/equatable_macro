...
macro class Equatable implements ClassDeclarationsMacro, ClassDefinitionMacro {
  ...
  /// Defaults to `false`. else take a Global EquatableConfig where we have our default config for stringify else deafult is false 
  ...
  const Equatable({ bool? stringify, }) : _stringify = stringify;//cant remove const from constructor if and also final from the stringfy method as annotation must have const constructor 
    // bool get stringify => _stringify ?? EquatableConfig.global.stringify ??false;

    bool get stringify {
      return _stringify  ?? false;
    }
  
 set stringify(bool stringify) => stringify = stringify;
